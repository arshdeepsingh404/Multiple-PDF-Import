...
* **Individual DWG Exports**: Convert PDF pages directly into separate standalone AutoCAD drawing `.dwg` files in a designated folder.
...
For the simplest experience, download the Multiple PDF Import add-in directly from the Autodesk App Store. The installer handles all configuration automatically.
...
2. Download the **MultiplePDFImport.bundle** zipped folder.
...
Click **Add PDF Files** to select the documents you want to process.
...
| Option | Description |
| :--- | :--- |
| **Import each page as Group** | Puts imported geometry for each page into a separate AutoCAD Group. |
| **Import each page as Block** | Creates an AutoCAD Block Definition for each imported page. |
| **All Pages / Custom Range** | Limits import to specific pages of multi-page PDFs. |
| **Imperial / Metric Radios** | Updates the Paper Size dropdown list with appropriate formats. |
